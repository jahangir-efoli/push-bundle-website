...
- **2026-07-23** — **Rebrand violet → blue+cyan (match the logo).** The earlier electric-violet primary clashed with the blue/cyan logo. Repointed the brand primitives (token layer only) to **blue + cyan** with **coral** as the warm accent; signature gradient is now the requested **cyan → teal → deep blue** (`#4DE8E9 → #50B2B3 → #1738B7`). Primary is a vivid blue (`#1e50e0`, white text ~7:1). All white-on-solid pairs re-verified AA; **155 tests pass** incl. axe light+dark. (Open item from the same feedback: the bundle-mockup uses colour dots, not real product images — needs real product art, §9 content-supply.)
